...
<section class="py-4 px-3 bg-white rounded shadow-sm" style="max-width: 600px; margin: 0 auto;">
  <form action="{{ route('murid.update', $student->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
      ...
      <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $student->user->name ?? '') }}" autofocus>
      @error('name')
    ...
    <div class="mb-3">
      ...
      <select name="gender" id="gender" class="form-select">
        <option value="0" {{ old('gender', $student->gender) == 0 ? 'selected' : '' }}>Laki-laki</option>
        <option value="1" {{ old('gender', $student->gender) == 1 ? 'selected' : '' }}>Perempuan</option>
      </select>
      @error('gender')
        <div class="text-danger small">{{ $message }}</div>
      @enderror
    ...
    <div class="mb-3">
      ...
      <input type="text" name="palace_of_birth" id="palace_of_birth" class="form-control" value="{{ old('palace_of_birth', $student->palace_of_birth) }}">
    ...
    <div class="mb-3">
      ...
      @if($student->photo)
        <small class="d-block mt-1">Foto saat ini: <a href="{{ asset('storage/'.$student->photo) }}" target="_blank">Lihat Foto</a></small>
      @endif
      @error('photo')
        <div class="text-danger small">{{ $message }}</div>
      @enderror
